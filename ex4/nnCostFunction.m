...
a1 = [onesRow X];
z2 = a1 * Theta1';
a2 = [onesRow sigmoid(z2)];
z3 = a2 * Theta2';
h = sigmoid(z3);

%Translating Y into a vector of 1's and 0's
...
% Remove the bias Theta value as we don't regularize with it
Theta1NoBias = Theta1(:,2:size(Theta1,2));
Theta2NoBias = Theta2(:,2:size(Theta2,2));
% Regularize the cost
J = J + lambda * ( ( sum( sumsq ( Theta1NoBias ) ) + sum ( sumsq ( Theta2NoBias ) ) ) / ( 2 * m ) )
...
for t=1:m,
	...
	delta_3 = a3 - newY(:,t); % y(k) trick - getting columns of t element
